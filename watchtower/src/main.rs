...
use {
    ...
    lunul_cli_output::display::format_labeled_address,
    lunul_metrics::{datapoint_error, datapoint_info},
    lunul_notifier::{NotificationType, Notifier},
    lunul_rpc_client::rpc_client::RpcClient,
    lunul_rpc_client_api::{client_error, response::RpcVoteAccountStatus},
    lunul_sdk::{
        hash::Hash,
...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `lunul-watchtower`:
        ...
        define environment variable before running `lunul-watchtower`:
        ...
        .arg(
            Arg::with_name("name_suffix")
                ...
                .takes_value(true)
                .default_value("")
                .help("Add this string into all notification messages after \"lunul-watchtower\"")
        )
        .get_matches();
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    lunul_metrics::set_panic_hook("watchtower", /*version:*/ None);
    ...
    loop {
        ...
        if let Some((failure_test_name, failure_error_message)) = &failure {
            ...
            if !last_notification_msg.is_empty() {
                ...
                info!("{}", all_clear_msg);
                notifier.send(
                    &format!("lunul-watchtower{}: {}", config.name_suffix, all_clear_msg),
