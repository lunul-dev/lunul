...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "lunul", "cli", "config.yml"]);
...
pub struct Config {
    ...
    ///
    /// [rpcdocs]: https://lunul.com/docs/core/clusters
    ...
    /// documentation for `lunul_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `lunul_clap_utils::keypair::DefaultSigner`.
    ...
    /// `lunul_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "lunul", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet-beta.lunul.com".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.lunul.com"),
            "ws://api.devnet.lunul.com/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.lunul.com"),
            "wss://api.devnet.lunul.com/".to_string()
        );
