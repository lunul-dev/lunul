use {
    ...
    lunul_clap_utils::{self, input_parsers::*, keypair::*},
    lunul_cli_config::ConfigInput,
    lunul_cli_output::{
        display::println_name_value, CliSignature, CliValidatorsSortOrder, OutputFormat,
    },
    lunul_remote_wallet::remote_wallet::RemoteWalletManager,
    ...
    lunul_rpc_client_nonce_utils::blockhash_query::BlockhashQuery,
    lunul_sdk::{
        clock::{Epoch, Slot},
    ...
    lunul_vote_program::vote_state::VoteAuthorize,
    std::{
...
pub enum CliError {
    ...
    #[error(transparent)]
    InvalidNonce(lunul_rpc_client_nonce_utils::Error),
...
impl From<lunul_rpc_client_nonce_utils::Error> for CliError {
    fn from(error: lunul_rpc_client_nonce_utils::Error) -> Self {
        match error {
            lunul_rpc_client_nonce_utils::Error::Client(client_error) => {
...
pub fn parse_command(
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            .gen_completions_to("lunul", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
        ...
        ("deploy", Some(_matches)) => clap::Error::with_description(
            "`lunul deploy` has been replaced with `lunul program deploy`",
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of lunul-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
...
mod tests {
    use {
        super::*,
        serde_json::json,
        lunul_rpc_client::mock_sender_for_cli::SIGNATURE,
        lunul_rpc_client_api::{
            request::RpcRequest,
            response::{Response, RpcResponseContext},
        },
        lunul_rpc_client_nonce_utils::blockhash_query,
        lunul_sdk::{
            pubkey::Pubkey,
            signature::{
        ...
        lunul_transaction_status::TransactionConfirmationStatus,
    };
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(lunul_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_process_command() {
        ...
        let new_authorized_pubkey = lunul_sdk::pubkey::new_rand();
        ...
        let bob_keypair = Keypair::new();
        let bob_pubkey = bob_keypair.pubkey();
        let custodian = lunul_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = lunul_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = lunul_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = lunul_sdk::pubkey::new_rand();
