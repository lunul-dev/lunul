...
use {
    ...
    lunul_ledger::{blockstore::Blockstore, shred::Shred},
    lunul_measure::measure::Measure,
    lunul_metrics::{inc_new_counter_error, inc_new_counter_info},
    lunul_poh::poh_recorder::WorkingBankEntry,
...
pub enum Error {
    #[error(transparent)]
    Blockstore(#[from] lunul_ledger::blockstore::BlockstoreError),
    #[error(transparent)]
    ClusterInfo(#[from] lunul_gossip::cluster_info::ClusterInfoError),
    ...
    #[error(transparent)]
    TransportError(#[from] lunul_sdk::transport::TransportError),
    ...
    fn new(
        ...
        thread_hdls.extend(socks.into_iter().map(|sock| {
            ...
            let run_transmit = move || loop {
                ...
                let res = Self::handle_error(res, "lunul-broadcaster-transmit");
                if let Some(res) = res {
                    return res;
                }
            };
        ...
        thread_hdls.extend(
            repeat_with(|| {
                ...
                let run_record = move || loop {
                    ...
                    let res = Self::handle_error(res, "lunul-broadcaster-record");
                    if let Some(res) = res {
                        return res;
                    }
                };
        ...
        let retransmit_thread = Builder::new()
            ...
            .spawn(move || loop {
                if let Some(res) = Self::handle_error(
                    ...
                    "lunul-broadcaster-retransmit-check_retransmit_signals",
                ) {
                    return res;
                }
            })
