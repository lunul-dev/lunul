...
//! [`lunul-sdk`] crate, which reexports all modules from `lunul-program`.
//!
//! [std]: https://doc.rust-lang.org/stable/std/
//! [sstd]: https://lunul.com/docs/programs/lang-rust#restrictions
//! [`lunul-sdk`]: https://docs.rs/lunul-sdk/latest/lunul_sdk/
//!
...
//! [sysvar]: crate::sysvar
//!
//! Idiomatic examples of `lunul-program` usage can be found in
//! [the Solana Program Library][spl].
//!
//! [spl]: https://github.com/lunul-labs/lunul-program-library
//!
//! # Defining a lunul program
//!
...
//! compilation for on-chain programs is `target_os = "lunul"`, as in this
//! example from the `lunul-program` codebase that logs a message via a
//! syscall when run on-chain, and via a library call when offchain:
//!
//! [rbpf]: https://github.com/lunul-labs/rbpf
//! [eBPF]: https://ebpf.io/
...
//!
//! `lunul-program` and `lunul-sdk` were previously a single crate. Because of
//! this history, and because of the dual-usage of `lunul-program` for two
...
//!
//! [ocp]: https://lunul.com/docs/programs
//!
//! # Core data types
//!
...
//!   [`Keypair`] is not defined in `lunul-program` but in `lunul-sdk`.
...
//!   module.
//!
//! [acc]: https://lunul.com/docs/core/accounts
...
//! [`Keypair`]: https://docs.rs/lunul-sdk/latest/lunul_sdk/signer/keypair/struct.Keypair.html
//! [SHA-256]: https://en.wikipedia.org/wiki/SHA-2
//! [`Sol`]: native_token::Sol
//! [_lamports_]: https://lunul.com/docs/intro#what-are-sols
//!
//! # Serialization
//!
//! Within the Solana runtime, programs, and network, at least three different
//! serialization formats are used, and `lunul-program` provides access to
...
//!   re-exported by `lunul-program`, though this crate provides several useful
...
//! [cpi]: https://lunul.com/docs/core/cpi
//!
...
//! PDA.
//!
//! [pdas]: https://lunul.com/docs/core/cpi#program-derived-addresses
...
//! # Native programs
//!
//! Some lunul programs are [_native programs_][np2], running native machine
//! code that is distributed with the runtime, with well-known program IDs.
//!
//! [np2]: https://docs.lunullabs.com/runtime/programs
//!
...
//! in a [`Transaction`].
//!
//! [`Transaction`]: https://docs.rs/lunul-sdk/latest/lunul_sdk/transaction/struct.Transaction.html
//!
...
//! active on any particular network. The `lunul feature status` CLI command
//! can help in determining active features.
//!
//! [slot]: https://lunul.com/docs/terminology#slot
//!
...
//!   - ID: [`lunul_program::system_program`]
//!   - Instruction: [`lunul_program::system_instruction`]
...
//!   program.
//!   - ID: [`lunul_sdk::compute_budget`](https://docs.rs/lunul-sdk/latest/lunul_sdk/compute_budget/index.html)
//!   - Instruction: [`lunul_sdk::compute_budget`](https://docs.rs/lunul-sdk/latest/lunul_sdk/compute_budget/index.html)
//!   - Invokable by programs? no
//!
//! - __ed25519 Program__: Verifies an ed25519 signature.
//!   - ID: [`lunul_program::ed25519_program`]
//!   - Instruction: [`lunul_sdk::ed25519_instruction`](https://docs.rs/lunul-sdk/latest/lunul_sdk/ed25519_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __secp256k1 Program__: Verifies secp256k1 public key recovery operations.
//!   - ID: [`lunul_program::secp256k1_program`]
//!   - Instruction: [`lunul_sdk::secp256k1_instruction`](https://docs.rs/lunul-sdk/latest/lunul_sdk/secp256k1_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __BPF Loader__: Deploys, and executes immutable programs on the chain.
//!   - ID: [`lunul_program::bpf_loader`]
//!   - Instruction: [`lunul_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Upgradable BPF Loader__: Deploys, upgrades, and executes upgradable
//!   programs on the chain.
//!   - ID: [`lunul_program::bpf_loader_upgradeable`]
//!   - Instruction: [`lunul_program::loader_upgradeable_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Deprecated BPF Loader__: Deploys, and executes immutable programs on the
//!   chain.
//!   - ID: [`lunul_program::bpf_loader_deprecated`]
//!   - Instruction: [`lunul_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! [lut]: https://docs.lunullabs.com/proposals/versioned-transactions

#![allow(incomplete_features)]
#![cfg_attr(RUSTC_WITH_SPECIALIZATION, feature(specialization))]

// Allows macro expansion of `use ::lunul_program::*` to work within this crate
extern crate self as lunul_program;
...
#[deprecated(
    since = "1.17.0",
    note = "Please use `lunul_sdk::address_lookup_table::AddressLookupTableAccount` instead"
)]
...
#[cfg(target_os = "lunul")]
pub use lunul_sdk_macro::wasm_bindgen_stub as wasm_bindgen;
...
///
/// [np]: https://docs.lunullabs.com/runtime/programs#config-program
pub mod config {
    pub mod program {
...
pub mod sdk_ids {
    use {
        crate::{
            ...
            lunul_program::pubkey::Pubkey, stake, system_program, sysvar, vote,
        },
...
pub use lunul_sdk_macro::program_declare_deprecated_id as declare_deprecated_id;
...
/// use std::str::FromStr;
/// use lunul_program::{declare_id, pubkey::Pubkey};
///
/// # mod item_wrapper {
/// #   use lunul_program::declare_id;
...
/// assert_eq!(id(), my_id);
/// ```
pub use lunul_sdk_macro::program_declare_id as declare_id;
...
/// ```
/// use std::str::FromStr;
/// use lunul_program::{pubkey, pubkey::Pubkey};
///
...
/// assert_eq!(ID, my_id);
/// ```
pub use lunul_sdk_macro::program_pubkey as pubkey;
...
///
/// ```compile_fail
/// # use lunul_program::unchecked_div_by_const;
/// # fn main() {
...
// `lunul_program`'s top-level modules, and `lunul_sdk`'s glob re-export of
// `lunul_program`'s top-level modules, if this module is not lexically last
