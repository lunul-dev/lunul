...
//! instead use the [`lunul-program`] crate, the modules of which are
...
//! provided by `lunul-program`, see that crate's documentation for an
//! overview.
//!
//! [`lunul-program`]: https://docs.rs/lunul-program
//!
//! Many of the modules in this crate are primarily of use to the Solana runtime
//! itself. Additional crates provide capabilities built on `lunul-sdk`, and
//! many programs will need to link to those crates as well, particularly for
//! clients communicating with Solana nodes over RPC.
//!
//! Such crates include:
//!
//! - [`lunul-client`] - For interacting with a Solana node via the [JSON-RPC API][json].
//! - [`lunul-cli-config`] - Loading and saving the Solana CLI configuration file.
//! - [`lunul-clap-utils`] - Routines for setting up the CLI using [`clap`], as
...
//! [json]: https://lunul.com/docs/rpc
//! [`clap`]: https://docs.rs/clap

#![allow(incomplete_features)]
#![cfg_attr(RUSTC_WITH_SPECIALIZATION, feature(specialization))]

// Allows macro expansion of `use ::lunul_sdk::*` to work within this crate
extern crate self as lunul_sdk;

#[cfg(feature = "full")]
pub use signer::signers;
// These lunul_program imports could be *-imported, but that causes a bunch of
// confusing duplication in the docs due to a rustdoc bug. #26211
#[allow(deprecated)]
pub use lunul_program::address_lookup_table_account;
#[cfg(not(target_os = "lunul"))]
pub use lunul_program::program_stubs;
pub use lunul_program::{
...
pub use lunul_sdk_macro::declare_deprecated_id;
...
/// use std::str::FromStr;
/// use lunul_sdk::{declare_id, pubkey::Pubkey};
///
/// # mod item_wrapper {
/// #   use lunul_sdk::declare_id;
...
/// assert_eq!(id(), my_id);
/// ```
pub use lunul_sdk_macro::declare_id;
...
/// assert_eq!(ID, my_id);
/// ```
pub use lunul_sdk_macro::pubkey;
/// Convenience macro to define multiple static public keys.
pub use lunul_sdk_macro::pubkeys;
#[rustversion::since(1.46.0)]
pub use lunul_sdk_macro::respan;

// Unused `lunul_sdk::program_stubs!()` macro retained for source backwards compatibility with older programs
#[macro_export]
#[deprecated(
    since = "1.4.3",
