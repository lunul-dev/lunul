...
// see: https://github.com/lunul-labs/lunul/pull/24853
lazy_static! {
...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = lunul_perf::perf_libs::locate_perf_libs()
            {
                lunul_perf::perf_libs::append_to_ld_library_path(
...
/// Work consensus!)
///
/// The lunul core protocol currently requires an `Entry` to contain `transactions` that are
/// executable in parallel. Implemented in:
///
/// * For TPU: `lunul_core::banking_stage::BankingStage::process_and_record_transactions()`
/// * For TVU: `lunul_core::replay_stage::ReplayStage::replay_blockstore_into_bank()`
///
...
///
/// This enforcement is done via a call to `lunul_runtime::accounts::Accounts::lock_accounts()`
...
mod tests {
    use {
        super::*,
        lunul_perf::test_tx::{test_invalid_tx, test_tx},
        lunul_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
