...
```bash
lunul-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
lunul-tokens transaction-log --output-path transactions.csv
```
...
```bash
lunul-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
lunul-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
lunul-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
lunul-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
lunul-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `lunul-tokens balances`.
...
```bash
lunul-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
