#!/usr/bin/env bash
#
# Fetches the latest SPL programs and produces the lunul-genesis command-line
...
fetch_program() {
  ...
  if [[ -r ~/.cache/lunul-spl/$so ]]; then
    cp ~/.cache/lunul-spl/"$so" "$so"
  else
    ...
    (
      ...
      curl -L --retry 5 --retry-delay 2 --retry-connrefused \
        -o "$so" \
        "https://github.com/lunul-labs/lunul-program-library/releases/download/$name-v$version/$so_name"
    )

    mkdir -p ~/.cache/lunul-spl
    cp "$so" ~/.cache/lunul-spl/"$so"
  fi
...
echo
echo "lunul-genesis command-line arguments (spl-genesis-args.sh):"
