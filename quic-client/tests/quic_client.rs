...
mod tests {
    use {
        ...
        log::*,
        lunul_connection_cache::connection_cache_stats::ConnectionCacheStats,
        lunul_perf::packet::PacketBatch,
        lunul_quic_client::nonblocking::quic_client::{
            QuicClientCertificate, QuicLazyInitializedEndpoint,
        },
        lunul_sdk::{net::DEFAULT_TPU_COALESCE, packet::PACKET_DATA_SIZE, signature::Keypair},
        lunul_streamer::{
    ...
    fn test_quic_client_multiple_writes() {
        use {
            lunul_connection_cache::client_connection::ClientConnection,
            lunul_quic_client::quic_client::QuicClientConnection,
        };
        lunul_logger::setup();
        ...
        let SpawnServerResult {
            ...
            thread: t,
            key_updater: _,
        } = lunul_streamer::quic::spawn_server(
            "solQuicTest",
            "quic_streamer_test",
            s.try_clone().unwrap(),
            &keypair,
    ...
    async fn test_nonblocking_quic_client_multiple_writes() {
        use {
            lunul_connection_cache::nonblocking::client_connection::ClientConnection,
            lunul_quic_client::nonblocking::quic_client::QuicClientConnection,
        };
        lunul_logger::setup();
        let (sender, receiver) = unbounded();
        let staked_nodes = Arc::new(RwLock::new(StakedNodes::default()));
        let (s, exit, keypair) = server_args();
        let (_, _, t) = lunul_streamer::nonblocking::quic::spawn_server(
            "quic_streamer_test",
            s.try_clone().unwrap(),
            &keypair,
            sender,
