...
RELEASE_BASENAME="${RELEASE_BASENAME:=lunul-release}"
...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/lunul-install-init lunul-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml lunul-install-init-"$TARGET"* $MAYBE_TARBALLS; do
  ...
  if [[ -n $BUILDKITE ]]; then
    ...
    upload-s3-artifact "/lunul/$file" s3://release.lunul.com/"$CHANNEL_OR_TAG"/"$file"

    echo Published to:
    $DRYRUN ci/format-url.sh https://release.lunul.com/"$CHANNEL_OR_TAG"/"$file"

    if [[ -n $TAG ]]; then
      ci/upload-github-release-asset.sh "$file"
    fi
  elif [[ -n $TRAVIS ]]; then
    # .travis.yml uploads everything in the travis-s3-upload/ directory to release.lunul.com
