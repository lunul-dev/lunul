...
case $testName in
test-stable)
  _ ci/intercept.sh cargo test --jobs "$JOBS" --all --tests --exclude lunul-local-cluster ${V:+--verbose} -- --nocapture
  ;;
test-stable-sbf)
  ...
  # lunul-keygen required when building C programs
  ...
  # SBF lunul-sdk legacy compile test
  ...
  # expected number of lunul-program crate compilations is 4. There
  # should be 3 builds of lunul-program while 128bit crate is
  ...
  # lunul-program is performed when simulation crate is built. This
  # last compiled lunul-program is of different version, normally the
  # latest mainbeta release version.
  lunul_program_count=$(grep -c 'lunul-program v' cargo.log)
  rm -f cargo.log
  if ((lunul_program_count > 20)); then
      echo "Regression of build redundancy ${lunul_program_count}."
      echo "Review dependency features that trigger redundant rebuilds of lunul-program."
      exit 1
  fi
  ...
  tar cjvf "$sbf_dump_archive" "${sbf_target_path}"/{deploy/*.txt,sbf-lunul-lunul/release/*.so}
  exit 0
  ;;
test-stable-perf)
  if [[ $(uname) = Linux ]]; then
    ...
    # Force CUDA for lunul-core unit tests
  ...
  _ cargo test --package lunul-perf --package lunul-ledger --package lunul-core --lib ${V:+--verbose} -- --nocapture
...
test-wasm)
  ...
  ;;
test-docs)
  _ cargo test --jobs "$JOBS" --all --doc --exclude lunul-local-cluster ${V:+--verbose} -- --nocapture
  exit 0
  ;;
*)
...
(
  ...
  export RUST_LOG="lunul_metrics=warn,info,$RUST_LOG"
