...
spl() {
  (
    ...
    project_used_lunul_version=$(sed -nE 's/lunul-sdk = \"[>=<~]*(.*)\"/\1/p' <"token/program/Cargo.toml")
    echo "used lunul version: $project_used_lunul_version"
    if semverGT "$project_used_lunul_version" "$SOLANA_VER"; then
      echo "skip"
      return
    fi
