...
#[deprecated(
    since = "1.8.0",
    note = "Please use `lunul_sdk::stake::state` or `lunul_program::stake::state` instead"
)]
pub use lunul_sdk::stake::state::*;
use {
    lunul_program_runtime::{ic_msg, invoke_context::InvokeContext},
    lunul_sdk::{
...
pub fn delegate(
    ...
    if *vote_account.get_owner() != lunul_vote_program::id() {
...
pub(crate) fn deactivate_delinquent(
    ...
    if *delinquent_vote_account.get_owner() != lunul_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
    ...
    if *reference_vote_account.get_owner() != lunul_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
...
mod tests {
    use {
        super::*,
        proptest::prelude::*,
        lunul_program_runtime::with_mock_invoke_context,
        lunul_sdk::{
    ...
    fn test_authorized_authorize() {
        let staker = lunul_sdk::pubkey::new_rand();
    ...
    fn test_authorized_authorize_with_custodian() {
        ...
        let invalid_custodian = lunul_sdk::pubkey::new_rand();
        let mut authorized = Authorized::auto(&staker);
        let mut signers = HashSet::new();
        signers.insert(staker);
    ...
    fn test_dbg_stake_minimum_balance() {
        ...
        panic!(
            ...
            minimum_balance,
            minimum_balance as f64 / lunul_sdk::native_token::LAMPORTS_PER_SOL as f64
        );
    }
