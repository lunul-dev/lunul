use {
    ...
    log::*,
    lunul_clap_utils::input_parsers::{lamports_of_sol, value_of},
    lunul_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    lunul_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = lunul_cli_config::Config::default().keypair_path;

    lunul_logger::setup_with_default("lunul=info");
    lunul_metrics::set_panic_hook("faucet", /*version:*/ None);
