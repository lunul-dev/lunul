...
echo 'Building the lunul cli from source...'
...
cargo build -p lunul-cli

usage=$(cargo -q run -p lunul-cli -- -C ~/.foo --help | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')
...
while read -r subcommand rest; do
  ...
  if ((in_subcommands)); then
      section "$(cargo -q run -p lunul-cli -- help "$subcommand" | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')" "###" >> "$out"
  fi
done <<<"$usage">>"$out"
