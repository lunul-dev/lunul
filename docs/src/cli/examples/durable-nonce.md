...
[`recent_blockhash`](https://lunul.com/docs/core/transactions#recent-blockhash).
...
```bash
lunul-keygen new -o nonce-keypair.json
lunul create-nonce-account nonce-keypair.json 1
```
...
> [Full usage documentation](../usage.md#lunul-create-nonce-account)
...
```bash
lunul nonce nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#lunul-get-nonce)
...
```bash
lunul new-nonce nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#lunul-new-nonce)
...
```bash
lunul nonce-account nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#lunul-nonce-account)
...
```bash
lunul withdraw-from-nonce-account nonce-keypair.json ~/.config/lunul/id.json 0.5
```
...
> [Full usage documentation](../usage.md#lunul-withdraw-from-nonce-account)
...
```bash
lunul authorize-nonce-account nonce-keypair.json nonce-authority.json
```
...
> [Full usage documentation](../usage.md#lunul-authorize-nonce-account)
...
```bash
$ lunul-keygen new -o alice.json
$ lunul-keygen new -o nonce.json
$ lunul-keygen new -o bob.json
```
...
```bash
$ lunul create-nonce-account -k alice.json nonce.json 0.1
...
```bash
$ lunul transfer -k alice.json --blockhash expiredDTaxfagttWjQweib42b6ZHADSx94Tw8gHx11 bob.json 0.01
[2020-01-02T18:48:28.462911000Z ERROR lunul_cli::cli] Io(Custom { kind: Other, error: "Transaction \"33gQQaoPc9jWePMvDAeyJpcnSPiGUAdtVg8zREWv4GiKjkcGNufgpcbFyRKRrA25NkgjZySEeKue5rawyeH5TzsV\" failed: None" })
...
```bash
$ lunul nonce-account nonce.json
balance: 0.1 SOL
minimum balance required: 0.00136416 SOL
...
```bash
$ lunul transfer -k alice.json --blockhash F7vmkY3DTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7 --nonce nonce.json bob.json 0.01
