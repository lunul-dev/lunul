...
[Staking and Inflation FAQ](https://lunul.com/staking).
:::
...
the
[stake account address](https://lunul.com/docs/economics/staking/stake-accounts#account-address).
No need for a password or encryption here; this keypair will be discarded right
after creating the stake account.

```bash
lunul-keygen new --no-passphrase -o stake-account.json
```
...
```bash
lunul create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `lunul stake-account` command:
...
[Stake and withdraw authorities](https://lunul.com/docs/economics/staking/stake-accounts#understanding-account-authorities)
can be set when creating an account via the `--stake-authority` and
`--withdraw-authority` options, or afterward with the `lunul stake-authorize`
command. For example, to set a new stake authority, run:

```bash
lunul stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
lunul create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
`lunul create-address-with-seed`:

```bash
lunul create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`lunul create-stake-account`.
...
accounts with the `lunul validators` command:
...
`lunul delegate-stake`:

```bash
lunul delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `lunul stake-account` to observe the changes to the
stake account:
...
Once delegated, you can undelegate stake with the `lunul deactivate-stake`
command:

```bash
lunul deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `lunul withdraw-stake` command:

```bash
lunul withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `lunul split-stake` command:

```bash
lunul split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
