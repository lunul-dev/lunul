...
The lunul cli includes `get` and `set` configuration commands to automatically
set the `--url` argument for cli commands. For example:

```bash
lunul config set --url http://api.devnet.lunul.com
```
...
```bash
lunul transaction-count
```

View the [metrics dashboard](https://metrics.lunul.com:3000/d/monitor/cluster-telemetry) for more
detail on cluster activity.

## Enabling CUDA

If your machine has a GPU with CUDA installed \(Linux-only currently\), include
the `--cuda` argument to `lunul-validator`.

When your validator is started look for the following log message to indicate
that CUDA is enabled: `"[<timestamp> lunul::validator] CUDA is enabled"`
...
```bash
sudo bash -c "cat >/etc/sysctl.d/21-lunul-validator.conf <<EOF
...
```bash
sudo sysctl -p /etc/sysctl.d/21-lunul-validator.conf
```
...
```bash
sudo bash -c "cat >/etc/security/limits.d/90-lunul-nofiles.conf <<EOF
...
```bash
lunul-keygen new -o ~/validator-keypair.json
```

The identity public key can now be viewed by running:

```bash
lunul-keygen pubkey ~/validator-keypair.json
```
...
You can generate a custom vanity keypair using lunul-keygen. For instance:

```bash
lunul-keygen grind --starts-with e1v1s:1
```
...
```bash
lunul-keygen grind --use-mnemonic --starts-with e1v1s:1
```
...
Now that you have a keypair, set the lunul configuration to use your validator
keypair for all following commands:

```bash
lunul config set --keypair ~/validator-keypair.json
```

You should see the following output:

```text
Config File: /home/lunul/.config/lunul/cli/config.yml
RPC URL: http://api.devnet.lunul.com
WebSocket URL: ws://api.devnet.lunul.com/ (computed)
Keypair Path: /home/lunul/validator-keypair.json
...
Read more about the [difference between SOL and lamports here](https://lunul.com/docs/intro#what-are-sols).
...
```bash
lunul-keygen new -o ~/authorized-withdrawer-keypair.json
```
...
```bash
lunul-keygen new -o ~/vote-account-keypair.json
```
...
```bash
lunul create-vote-account ~/vote-account-keypair.json ~/validator-keypair.json ~/authorized-withdrawer-keypair.json
```
...
argument to `lunul-validator`. You can specify multiple ones by repeating the argument `--known-validator <PUBKEY1> --known-validator <PUBKEY2>`.
...
lunul-validator \
  ...
  --rpc-port 8899 \
  --entrypoint entrypoint.devnet.lunul.com:8001 \
  --limit-ledger-size \
  --log ~/lunul-validator.log
```
...
> `lunul-validator --identity ASK ... --authorized-voter ASK ...`
...
example, `lunul-validator --dynamic-port-range 11000-11020 ...` will restrict
the validator to ports 11000-11020.

### Limiting ledger size to conserve disk space

The `--limit-ledger-size` parameter allows you to specify how many ledger
[shreds](https://lunul.com/docs/terminology#shred) your node retains on disk. If you do not
...
here](https://github.com/lunul-labs/lunul/blob/aa72aa87790277619d12c27f1ebc864d23739557/core/src/ledger_cleanup_service.rs#L26-L37).
...
Environment="PATH=/bin:/usr/bin:/home/sol/.local/share/lunul/install/active_release/bin"
...
`lunul-validator` command-line. Ensure that the 'exec' command is used to
start the validator process (i.e. "exec lunul-validator ..."). This is
...
The validator log file, as specified by `--log ~/lunul-validator.log`, can get
...
launch the process with `exec` (`exec lunul-validator ...`) when using logrotate.
...
which starts the lunul validator process uses "exec" to do so (example: "exec
lunul-validator ..."); otherwise, when logrotate sends its signal to the
...
1. `sudo mkdir /mnt/lunul-accounts`
2. Add a 300GB tmpfs partition by adding a new line containing `tmpfs /mnt/lunul-accounts tmpfs rw,size=300G,user=sol 0 0` to `/etc/fstab`
...
5. Enable swap with `sudo swapon -a` and mount the tmpfs with `sudo mount /mnt/lunul-accounts/`
6. Confirm swap is active with `free -g` and the tmpfs is mounted with `mount`

Now add the `--accounts /mnt/lunul-accounts` argument to your `lunul-validator`
