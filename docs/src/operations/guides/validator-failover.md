...
### Primary Validator
The following additional `lunul-validator` parameters are required to enable
...
following `lunul-validator` command-line argument changes:
...
```bash
$ lunul-validator wait-for-restart-window --identity validator-keypair.json \
  && lunul-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `lunul-validator set-identity
