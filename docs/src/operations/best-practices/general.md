...
From within the Solana CLI, you can execute the `lunul-validator` command with
the `--help` flag to get a better understanding of the flags and sub commands
available.

```
lunul-validator --help
```
...
[leader slot](https://lunul.com/docs/terminology#leader-schedule) is the time
...
Assuming you are ready to restart, you may use the `lunul-validator exit`
...
your `lunul-validator` binary to your specific hardware.
...
`lunul-install` command.

Assuming you want to install Solana version `1.14.17`, you would execute the
following:

```
lunul-install init 1.14.17
```

This command downloads the executable for `1.14.17` and installs it into a
`.local` directory. You can also look at `lunul-install --help` for more
options.

> **Note** this command only works if you already have the lunul cli installed.
> If you do not have the cli installed, refer to
> [install lunul cli tools](../../cli/install.md)

### Restart

For all install methods, the validator process will need to be restarted before
the newly installed version is in use. Use `lunul-validator exit` to restart
...
`lunul-validator` command:

```
--no-snapshot-fetch
```

If you use this flag with the `lunul-validator` command, make sure that you run
`lunul catchup <pubkey>` after your validator starts to make sure that the
...
address of the validator in using the `lunul gossip` command. In the example
below, `5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on` is the pubkey of one of my
known validators:

```
lunul gossip | grep 5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on
```
...
running the `lunul-validator` software. How much SOL you should store there is
...
```
lunul balance validator-keypair.json
```

> **Note** `lunul-watchtower` can monitor for a minimum validator identity
...
`lunul withdraw-from-vote-account` on a trusted computer. For example, on a
...
```
lunul withdraw-from-vote-account \
...
`lunul withdraw-from-vote-account --help`.
