use {
    ...
    itertools::Itertools,
    log::*,
    lunul_accounts_db::accounts_index::{AccountIndex, AccountSecondaryIndexes},
    lunul_clap_utils::{
        input_parsers::{pubkey_of, pubkeys_of, value_of},
        input_validators::normalize_to_url_if_moniker,
    },
    lunul_core::consensus::tower_storage::FileTowerStorage,
    lunul_faucet::faucet::run_local_faucet_with_port,
    lunul_rpc::{
...
fn main() {
    ...
    let version = lunul_version::version!();
    ...
    lunul_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    info!("{} {}", crate_name!(), lunul_version::version!());
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        lunul_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        lunul_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
    ...
    if TestValidatorGenesis::ledger_exists(&ledger_path) {
        ...
        println_name_value(
            "\nNotice!",
            "No wallet available. `lunul airdrop` localnet SOL after creating one\n",
        );
    }
