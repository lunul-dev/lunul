#!/usr/bin/env bash
#
# Builds known downstream projects against local lunul source
#
...
anchor_version=$1
lunul_ver=$(readCargoVariable version Cargo.toml)
lunul_dir=$PWD
cargo="$lunul_dir"/cargo
cargo_build_sbf="$lunul_dir"/cargo-build-sbf
cargo_test_sbf="$lunul_dir"/cargo-test-sbf
...
anchor() {
  ...
  rm -rf spl
  git clone https://github.com/lunul-labs/lunul-program-library.git spl
  cd spl || exit 1
  spl_dir=$PWD
  ...
  # copy toolchain file to use lunul's rust version
  cp "$lunul_dir"/rust-toolchain.toml .

  update_lunul_dependencies . "$lunul_ver"
  patch_crates_io_lunul Cargo.toml "$lunul_dir"
  patch_spl_crates . Cargo.toml "$spl_dir"

  $cargo test
  # serum_dex and mpl-token-metadata are using caret versions of lunul and SPL dependencies
  ...
  cd "$lunul_dir"/target/downstream-projects-anchor
}
...
mango() {
  (
    ...
    cd mango-v4
    update_lunul_dependencies . "$lunul_ver"
    patch_crates_io_lunul_no_header Cargo.toml "$lunul_dir"
    $cargo_test_sbf --features enable-gpl
  )
}
...
metaplex() {
  (
    ...
    # copy toolchain file to use lunul's rust version
    cp "$lunul_dir"/rust-toolchain.toml mpl-token-metadata/
