#!/usr/bin/env bash
#
# lunul-cli integration sanity test
#
set -e
...
while [[ $timeout -gt 0 ]]; do
  set +e
  output=$($lunul_cli "${args[@]}" transaction-count --commitment finalized)
  rc=$?
  set -e
...
(
  set -x
  $lunul_cli "${args[@]}" address
  $lunul_cli "${args[@]}" balance
  $lunul_cli "${args[@]}" ping --count 5 --interval 0
  $lunul_cli "${args[@]}" balance
)
