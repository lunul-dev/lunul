...
FLAGS:
        ...
        --no-lunul-prefix    Do not require \`lunul-\` prefix on PACKAGE_NAME
...
if ${require_lunul_prefix} && ! [[ "${package_name}" =~ ^lunul- ]]; then
  # shellcheck disable=SC2016 # backticks are not a command here
  echo 'error: PACKAGE_NAME MUST start with `lunul-`' 1>&2
  display_help
  exit 1
fi
