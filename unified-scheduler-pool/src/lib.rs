...
//!
//! This crate implements 3 lunul-runtime traits (`InstalledScheduler`, `UninstalledScheduler` and
...
//! `lunul-ledger`'s helper function called `execute_batch()`.
...
use {
    ...
    log::*,
    lunul_ledger::blockstore_processor::{
    ...
    lunul_sdk::transaction::{Result, SanitizedTransaction},
    lunul_unified_scheduler_logic::Task,
...
// SchedulerPool must be accessed as a dyn trait from lunul-runtime, because SchedulerPool
// contains some internal fields, whose types aren't available in lunul-runtime (currently
...
mod tests {
    ...
    fn test_scheduler_schedule_execution_failure() {
        ...
        assert_matches!(
            ...
            Some((
                Err(lunul_sdk::transaction::TransactionError::AccountNotFound),
                _timings
            ))
        );
    }
