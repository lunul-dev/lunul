...
local|tar|skip)
  ...
  lunul_cli=lunul
  lunul_gossip=lunul-gossip
  lunul_install=lunul-install
  ;;
*)
...
(
  set -x
  $lunul_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $lunul_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: lunul-install test"
