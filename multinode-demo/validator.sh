...
if [[ -n $SOLANA_CUDA ]]; then
  program=$lunul_validator_cuda
else
  program=$lunul_validator
fi
...
wallet() {
  (
    set -x
    $lunul_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
...
rpc_url=$($lunul_gossip --allow-private-addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $lunul_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $lunul_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $lunul_keygen new --no-passphrase -so "$authorized_withdrawer"
